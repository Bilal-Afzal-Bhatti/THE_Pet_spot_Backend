...
// Helper: safely parse JSON array fields sent via FormData
const parseOverviewPoints = (value: any): { title: string; description: string }[] => {
  if (!value) return [];
...
export const createAdminBreed = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const breed = await AdminBreedModel.create({
      ...
      overviewPoints: parseOverviewPoints(body.overviewPoints),
      breedInfoPoints: parseOverviewPoints(body.breedInfoPoints),
      commonNicknames: body.commonNicknames || "",
      trainability: body.trainability || "",
      shedding: body.shedding || "",
      grooming: body.grooming || "",
      breedType: body.breedType || "",
      size: body.size || "",
      image: imageUrl,
...
export const updateAdminBreed = asyncHandler(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (body.overviewPoints !== undefined) {
      breed.overviewPoints = parseOverviewPoints(body.overviewPoints);
    }
    if (body.breedInfoPoints !== undefined) {
      breed.breedInfoPoints = parseOverviewPoints(body.breedInfoPoints);
    }
    if (body.commonNicknames !== undefined) breed.commonNicknames = body.commonNicknames;
    if (body.trainability !== undefined) breed.trainability = body.trainability;
    if (body.shedding !== undefined) breed.shedding = body.shedding;
    if (body.grooming !== undefined) breed.grooming = body.grooming;
    if (body.breedType !== undefined) breed.breedType = body.breedType;
    if (body.size !== undefined) breed.size = body.size;
